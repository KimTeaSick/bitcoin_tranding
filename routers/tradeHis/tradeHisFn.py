...
class TradeHisFn():
  ...
  async def orderListPageCount(self, bit):
    value = await bit.mysql.Select(orderListCountSql)
    count = int(value[0][0]) / int(self.count)
    return round(count)

  async def getOrderList(self, page, bit):
    prev = "0" if page == 1 else str((int(page) - 1) * 14)
    selectData = await bit.mysql.Select(orderListSql(self.count, prev))
    orderList = []
