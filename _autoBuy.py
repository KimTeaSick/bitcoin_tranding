...
async def autoBuy(idx):
  try:
    db = SessionLocal()
    db: Session
    active_user = db.query(models.USER_T).filter(models.USER_T.idx == idx).first()
    ...
    for coin in coins['recommends']:
      print("use option name ::: :::", useRecommendOPtion.name)
    ...
    for coin in sortedCoins:
        try:
            ...
            if coin_ask_price == 'pass': continue
            print('coin ask price ::: ::: ', coin_ask_price)
            print('-----------------------------------------------------------------------------------------------------------------')
...
asyncio.run(autoBuy(sys.argv[1]))