...
def buy(active_user, db):
    try:
        bithumb = Bithumb(active_user.public_key, active_user.secret_key)
        # 사용 db 가져오기
        possession_coins = db.query(models.possessionCoin).filter(
            models.possessionCoin.user_idx == active_user.idx).all()
        ...
        db.query(models.recommendList).filter(models.recommendList.user_idx == active_user.idx).delete()
        # prevCoin = db.query(models.recommendList).filter(models.recommendList.user_idx == active_user.idx).delete()
        # if len(prevCoin) != 0:
        ...
        for coin in coins['recommends']:
            print("useRecommendOPtion.name :::: :::: ", useRecommendOPtion.name)
...
def buy_t():
    try:
        ...
        for active_user in active_users:
            db = SessionLocal()
            db: Session
            t = threading.Thread(target=buy, args=([active_user, db]))
            t.start()
    except Exception as e:
        print("buy_t Error ::::", e)
        db.rollback()
