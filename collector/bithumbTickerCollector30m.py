...
# 30분 전 데이터 ~ 최신 데이터 찾아서 dataframe 변환
now = str(datetime.datetime.now() - datetime.timedelta(minutes=30))
findidx = now[:-10]
...
for performance in performanceList:
    dfList.append({'coin_name': performance.coin_name, 'S_time': performance.S_time, 'time': performance.time, 'Open': performance.Open, 'Close': performance.Close, 'High': performance.High, 'Low': performance.Low,'Avg_price': performance.Avg_price, 'Transaction_amount': performance.Transaction_amount, 'Volume': performance.Volume, 'Disparity': performance.Disparity, 'Ask_price': performance.Ask_price})
...
print(bulkList)
db.bulk_insert_mappings(models.coinPrice30M, bulkList)
db.commit()
