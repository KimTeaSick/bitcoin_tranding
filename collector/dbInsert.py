...
for coin in coinList:
    currentPrice = models.coincurrentPrice()
    currentPrice.coin_name = coin.coin_name
    currentPrice.S_time = 0
    currentPrice.time = 0
    currentPrice.Open = 0
    currentPrice.Close = 0
    currentPrice.High = 0
    currentPrice.Low = 0
    currentPrice.Ask_price = 0
    currentPrice.Avg_price = 0
    currentPrice.Volume = 0
    currentPrice.Transaction_amount = 0
    currentPrice.Disparity = 0
        ...
        for coin in self.coinList:
            ...
            try:
                ...
                if int(dt) == int(unix_timestamp):
                    currentPrice.append({'S_time' : int(data2[0]/1000), 'Open': data2[1], 'Close': data2[2], 'High': data2[3], 'Low':data2[4], 'Volume': data2[5], 'coin_name': data2[6], 'time': data2[7]})

                else:
                    emptyCnt = int(((unix_timestamp - dt) / 60) /1)
                    currentPrice.append({'S_ime' : unix_timestamp, 'Open': data2[1], 'Close': data2[2], 'High': data2[3], 'Low': data2[4], 'Volume': data2[5], 'coin_name': data2[6], 'time': datetime.datetime.fromtimestamp(unix_timestamp), 'empty_count':emptyCnt})
...
if __name__ == '__main__':
    ...
    db.bulk_insert_mappings(models.coinCurrentPrice, currentPrice)
    db.commit()
