...
unix_timestamp = int(now.timestamp() /60 ) * 60 - 300# 3분전 변환
...
class MyThread(threading.Thread):
    ...
    def run(self):
        ...
        for coin in self.coinList:
            if coin.delflag == 1:
                continue
            url = f"https://api.bithumb.com/public/candlestick/{coin.coin_name}/5m"
            ...
            try:
                ...
                # 빈 시간 0 채움
                df = df.set_index('time').resample('5T').asfreq()
                df = df.fillna(0)
                ...
                if int(dt) == int(unix_timestamp):
                    ...
                    emptyCnt = int(((unix_timestamp - dt) / 60) /5)
...
if __name__ == '__main__':
    ...
    db.bulk_insert_mappings(models.coin5MPrice, currentPrice)
    db.commit()
