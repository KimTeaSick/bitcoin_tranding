...
unix_timestamp = int(now.timestamp() /60 ) * 60 - 21600# 1시간전 변환
...
class MyThread(threading.Thread):
    ...
    def run(self):
        ...
        for coin in self.coinList:
            if coin.delflag == 1:
                continue
            url = f"https://api.bithumb.com/public/candlestick/{coin.coin_name}/6h"
            ...
            try:
                ...
                if int(dt) == int(unix_timestamp):
                    ...
                    emptyCnt = int(((unix_timestamp - dt) / 60) /360)
...
if __name__ == '__main__':
    ...
    db.bulk_insert_mappings(models.coin6HPrice, currentPrice)
    db.commit()
